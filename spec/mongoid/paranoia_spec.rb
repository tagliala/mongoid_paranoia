...
describe Mongoid::Paranoia do
  context 'when configuring the paranoid_field setting' do
    before do
  ...
  describe '#destroy' do
    ...
    context 'when the document has a dependent: :restrict relation' do
      ...
      before do
        begin
          post.destroy
        rescue Mongoid::Errors::DeleteRestriction # rubocop:disable Lint/HandleExceptions
        end
      end
  ...
  describe '#restore_relations' do
    ...
    let!(:para_has_many)    { Array.new(2) { subject.para_has_many.create }   }
    let!(:para_habtm)       { Array.new(3) { subject.para_habtm.create }      }
    ...
    let!(:para_embeds_many) { Array.new(2) { subject.para_embeds_many.build } }

    let!(:norm_has_one)     { subject.norm_has_one = NormHasOne.create }
    let!(:norm_has_many)    { Array.new(2) { subject.norm_has_many.create }   }
    let!(:norm_habtm)       { Array.new(3) { subject.norm_habtm.create }      }
    ...
    let!(:norm_embeds_many) { Array.new(2) { subject.norm_embeds_many.build } }
    ...
    describe 'restores paranoid associations' do
      before { prepare }
    ...
    describe 'does not affect embedded paranoid documents' do
      before { prepare }

      it { expect { subject.restore_relations }.not_to change(subject, :para_embeds_one) }
      it { expect { subject.restore_relations }.not_to(change { subject.para_embeds_many.count }) }
    end

    describe 'does not affect non-paranoid documents' do
      before { prepare }

      it { expect { subject.restore_relations }.not_to change(NormHasOne, :count) }
      it { expect { subject.restore_relations }.not_to change(NormHasMany, :count) }
      it { expect { subject.restore_relations }.not_to change(NormHabtm, :count) }
      it { expect { subject.restore_relations }.not_to change(NormBelongsTo, :count) }
      it { expect { subject.restore_relations }.not_to change(subject, :norm_embeds_one) }
      it { expect { subject.restore_relations }.not_to(change { subject.norm_embeds_many.count }) }
    end
    ...
    context 'with recursion' do
      ...
      let!(:para_habtm_norm_has_many) { Array.new(2) { subject.para_habtm.first.norm_has_many  = NormHasMany.create } } # not restored
      let!(:para_habtm_para_has_many) { Array.new(3) { subject.para_habtm.second.para_has_many = ParaHasMany.create } } # restored

      # Untestable due to infinite recursion condition in #destroy
      # let!(:para_habtm_norm_habtm)    { Array.new(3) { subject.para_habtm.second.norm_habtm.create } } # not restored
      # let!(:para_habtm_recursive)     { Array.new(2) { subject.para_habtm.first.recursive.create }   } # restored
